...
r5rsTestTree = testGroup "Language/R5RS.hs" $ [
  ...
  testGroup "identifier" $ [
    ...
    testGroup "number" $ [
      ...
      let s = "-0" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
      ...
      let s = "-12345" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Minus (UInt $ UInteger 12345 0))),
      let s = "12345" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Plus (UInt $ UInteger 12345 0))),

      let s = "-12345/5" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
      ...
      let s = "-12345.0" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
      ...
      let s = ".0" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Plus (UDecimal (UInteger 0 0)
                                                         (Left (UInteger 0 0))
                                                         Nothing))),
      let s = "-.569" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Minus (UDecimal (UInteger 0 0)
                                                         (Left (UInteger 569 0))
                                                         Nothing))),
      let s = "1e10" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Plus (UDecimal (UInteger 1 0)
                                                         (Left (UInteger 0 0))
                                                         (Just $ Suffix PDefault Plus 10)))),
      let s = "1e-10" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Plus (UDecimal (UInteger 1 0)
                                                         (Left (UInteger 0 0))
                                                         (Just $ Suffix PDefault Minus 10)))),
      let s = "1s10" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Plus (UDecimal (UInteger 1 0)
                                                         (Left (UInteger 0 0))
                                                         (Just $ Suffix PShort Plus 10)))),
      let s = "1f10" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Plus (UDecimal (UInteger 1 0)
                                                         (Left (UInteger 0 0))
                                                         (Just $ Suffix PSingle Plus 10)))),
      let s = "1d10" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Plus (UDecimal (UInteger 1 0)
                                                         (Left (UInteger 0 0))
                                                         (Just $ Suffix PDouble Plus 10)))),
      let s = "1l10" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
                      ComplexReal (SReal Plus (UDecimal (UInteger 1 0)
                                                         (Left (UInteger 0 0))
                                                         (Just $ Suffix PLong Plus 10)))),
        
      let s = "1+i" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
      ...
      let s = "0.5+i" in testCase (show s) $ tparse R5.number s @?= (Right $ SchemeNumber Nothing Nothing $
